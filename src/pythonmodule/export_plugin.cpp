...
template<>
std::shared_ptr<gmxapi::MDModule> PyRestraint<plugin::RestraintModule<plugin::EBMetaDRestraint>>::getModule()
{
    return shared_from_this();
}
...
class EBMetaDRestraintBuilder
{
    public:
        explicit EBMetaDRestraintBuilder(py::object element)
        {
            ...
            auto historicalDistancesFilename = pybind11::cast<std::string>(parameter_dict["historical_data_filename"]);

            auto params = plugin::makeEBMetaDParams(forces,
                    distanceCounts,
                    binWidth,
                    samplePeriod,
                    historicalDistancesFilename,
                    minDist,
                    maxDist,
                    k);
        ...
        void build(py::object graph)
        {
            ...
            auto potential = PyRestraint<plugin::RestraintModule<plugin::EBMetaDRestraint>>::create(name_,
                                                                                                     siteIndices_,
                                                                                                     params_,
                                                                                                     resources);
...
std::unique_ptr<EBMetaDRestraintBuilder> createEBMetaDBuilder(const py::object element)
{
    using gmx::compat::make_unique;
...
PYBIND11_MODULE(myplugin, m) {
    ...
    //
    // Define Builder to be returned from ebmetad_restraint Python function defined further down.
    pybind11::class_<EBMetaDRestraintBuilder> ebmetadBuilder(m,
                                                               "EBMetaDBuilder");
    ebmetadBuilder.def("add_subscriber",
                        &EBMetaDRestraintBuilder::addSubscriber);
    ebmetadBuilder.def("build",
                        &EBMetaDRestraintBuilder::build);

    // Get more concise name for the template instantiation...
    using PyEBMetaD = PyRestraint<plugin::RestraintModule<plugin::EBMetaDRestraint>>;
    ...
    // API object to build.
    py::class_<PyEBMetaD, std::shared_ptr<PyEBMetaD >> ebmetad(m, "EBMetaDRestraint");
    // EnsembleRestraint can only be created via builder for now.
    ebmetad.def("bind",
                 &PyEBMetaD::bind,
                 "Implement binding protocol");
