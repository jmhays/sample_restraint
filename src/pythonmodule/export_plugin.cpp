...
PyRestraint<plugin::HarmonicModule>::getModule() {
  return shared_from_this();
}

template <>
std::shared_ptr<gmxapi::MDModule>
PyRestraint<plugin::RestraintModule<plugin::EnsembleRestraint>>::getModule() {
  return shared_from_this();
}
...
template <>
std::shared_ptr<gmxapi::MDModule>
PyRestraint<plugin::RestraintModule<plugin::EBMetaDRestraint>>::getModule() {
  return shared_from_this();
}
 ...
 *
 * Returned by create_builder(), translates the workflow operation into API
 * operations.
 */
...
public:
  explicit LinearRestraintBuilder(py::object element) {
    ...
    // It looks like we need some boilerplate exceptions for plugins so we have
    // something to raise if the element is invalid.
    assert(py::hasattr(element, "params"));

    // Params attribute should be a Python list
    py::dict parameter_dict = element.attr("params");
    // \todo Check for the presence of these dictionary keys to avoid
    ...
    auto params = plugin::makeLinearParams(alpha, target,
            samplePeriod, logging_filename);

    params_ = std::move(*params);

    // Note that if we want to grab a reference to the Context or its
    // communicator, we can get it here through element.workspec._context. We
    // need a more general API solution, but this code is in the Python bindings
    // code, so we know we are in a Python Context.
   ...
   *
   * \param subscriber Python object with a 'potential' attribute that is a
   * Python list.
   *
   * During build, an object is added to the subscriber's self.potential, which
   * is then bound with system.add_potential(potential) during the subscriber's
   * launch()
   */
...
//   *
//   * \param element a Python object implementing the gmx.workflow.WorkElement
//   * interface.
//   *
//   * It doesn't make sense to take a py::object here. We could take a serialized
//   * version of the element iff we also got a reference to the current context,
//   * but right now we use the gmx.workflow.WorkElement's reference to the
//   * WorkSpec, which has a reference to the Context, to get, say, the
//   * communicator. Arguably, a builder provided by the restraint shouldn't do
//   * such a thing.
//   *
//   * Either the builder / translator / DAG updater should be Context agnostic or
//   * should actually be implemented in the Context, in which case we need some
//   * better convention about what that translation should look like and what
//   * resources need to be provided by the module to do it.
//   */
//
//  explicit LinearRestraintBuilder(py::object element) {
//    // Params attribute should be a Python list
...
//    }
//
//    equilibriumPosition_ = py::cast<real>(parameter_dict["target"]);
//    springConstant_ = py::cast<real>(parameter_dict["alpha"]);
//    logging_
//  };
//
//  /*!
...
//   */
//  void build(py::object graph) {
//    auto potential = PyRestraint<plugin::LinearModule>::create(
//        siteIndices_, equilibriumPosition_, springConstant_);
//
//    auto subscriber = subscriber_;
...
public:
  ...
  void build(py::object graph) {
    auto potential = PyRestraint<plugin::HarmonicModule>::create(
        site1Index_, site2Index_, equilibriumPosition_, springConstant_);
...
public:
  explicit EnsembleRestraintBuilder(py::object element) {
    ...
    auto experimental =
        pybind11::cast<std::vector<double>>(parameter_dict["experimental"]);
    ...
    auto params = plugin::makeEnsembleParams(nbins, binWidth, minDist, maxDist,
                                             experimental, nSamples,
  ...
  void build(py::object graph) {
    ...
    auto potential =
        PyRestraint<plugin::RestraintModule<plugin::EnsembleRestraint>>::create(
...
public:
  explicit LinearStopRestraintBuilder(py::object element) {
    ...
    auto params = plugin::makeLinearStopParams(alpha, tolerance, target,
                                               samplePeriod, logging_filename);
...
class EBMetaDRestraintBuilder {
public:
  explicit EBMetaDRestraintBuilder(py::object element) {
    name_ = py::cast<std::string>(element.attr("name"));
    assert(!name_.empty());
    ...
    auto distanceCounts = pybind11::cast<std::vector<unsigned long>>(
        parameter_dict["distance_counts"]);
    auto samplePeriod = pybind11::cast<double>(parameter_dict["sample_period"]);
    auto k = pybind11::cast<double>(parameter_dict["k"]);

    auto historicalDistancesFilename =
        pybind11::cast<std::string>(parameter_dict["historical_data_filename"]);

    auto params = plugin::makeEBMetaDParams(
        forces, distanceCounts, binWidth, samplePeriod,
        historicalDistancesFilename, minDist, maxDist, k);
  ...
  void build(py::object graph) {
    ...
    auto potential =
        PyRestraint<plugin::RestraintModule<plugin::EBMetaDRestraint>>::create(
            name_, siteIndices_, params_, resources);
 ...
 */
std::unique_ptr<HarmonicRestraintBuilder>
createHarmonicBuilder(const py::object element) {
  std::unique_ptr<HarmonicRestraintBuilder> builder{
      new HarmonicRestraintBuilder(element)};
  return builder;
}
...
std::unique_ptr<EnsembleRestraintBuilder>
createEnsembleBuilder(const py::object element) {
  using gmx::compat::make_unique;
  auto builder = make_unique<EnsembleRestraintBuilder>(element);
  return builder;
}

std::unique_ptr<EBMetaDRestraintBuilder>
createEBMetaDBuilder(const py::object element) {
  using gmx::compat::make_unique;
  auto builder = make_unique<EBMetaDRestraintBuilder>(element);
  return builder;
}
...
PYBIND11_MODULE(myplugin, m) {
  ...
  // New plan: Instead of inheriting from gmx.core.MDModule, we can use a local
  // import of gmxapi::MDModule in both gmxpy and in extension modules. When
  // md.add_potential() is called, instead of relying on a binary interface to
  // the MDModule, it will pass itself as an argument to that module's bind()
  // method. Then, all MDModules are dependent only on libgmxapi as long as they
  // provide the required function name. This is in line with the Pythonic idiom
  // of designing interfaces around functions instead of classes.
  //
  // Example: calling md.add_potential(mypotential) in Python causes to be
  // called mypotential.bind(api_object), where api_object is a member of `md`
  // that is a type exposed directly from gmxapi with module_local bindings. To
  // interact properly, then, mypotential just has to be something with a bind()
  // method that takes the same sort of gmxapi object, such as is defined
  // locally. For simplicity and safety, this gmxapi object will be something
  // like class MdContainer { public: shared_ptr<Md> md; }; and the bind method
  // will grab and operate on the member pointer. It is possible to work with
  // the reference counting and such in Python, but it is easier and more
  // compatible with other Python extensions if we just keep the bindings as
  // simple as possible and manage object lifetime and ownership entirely in
  // C++.
  //
  // We can provide a header or document in gmxapi or gmxpy specifically with
  // the the set of containers necessary to interact with gmxpy in a
  // bindings-agnostic way, and in gmxpy and/or this repo, we can provide an
  // export function that provides pybind11 bindings.
  ...
  // API object to build.
  // We use a shared_ptr handle because both the Python interpreter and
  // libgromacs may need to extend the lifetime of the object.
  py::class_<PyRestraint<plugin::HarmonicModule>,
             std::shared_ptr<PyRestraint<plugin::HarmonicModule>>>
      harmonic(m, "HarmonicRestraint");

  // Deprecated constructor directly taking restraint paramaters.
  harmonic.def(py::init([](unsigned long int site1, unsigned long int site2,
                           real R0, real k) {
                 return PyRestraint<plugin::HarmonicModule>::create(
                     site1, site2, R0, k);
               }),
   ...
   * to have an add_subscriber(other_builder) method and a build(graph) method.
   * The build() method returns None or a launcher. A launcher has a signature
   * like launch(rank) and returns None or a runner.
   */
  m.def("create_restraint", [](const py::object element) {
    return createHarmonicBuilder(element);
  });
   //////////////////////////////////////////////////////////////////////////
...
//  // We use a shared_ptr handle because both the Python interpreter and
//  // libgromacs may need to extend the lifetime of the object.
//  py::class_<PyRestraint<plugin::LinearModule>,
//             std::shared_ptr<PyRestraint<plugin::LinearModule>>>
//      linear(m, "LinearRestraint");
//
//  // Deprecated constructor directly taking restraint paramaters.
//  linear.def(
//      py::init([](std::vector<unsigned long int> sites, real R0, real k) {
...
//        [](const py::object element) { return createLinearBuilder(element); });
  ...
  // Get more concise name for the template instantiation...
  using PyEnsemble =
      PyRestraint<plugin::RestraintModule<plugin::EnsembleRestraint>>;
  ...
  m.def("ensemble_restraint", [](const py::object element) {
  ...
  //
  // Define Builder to be returned from linearstop_restraint Python function
  // defined further down.
  ...
  //////////////////////////////////////////////////////////////////////////
  // Begin EBMetaDRestraint
  //
  // Define Builder to be returned from ebmetad_restraint Python function
  // defined further down.
  pybind11::class_<EBMetaDRestraintBuilder> ebmetadBuilder(m, "EBMetaDBuilder");
  ebmetadBuilder.def("add_subscriber", &EBMetaDRestraintBuilder::addSubscriber);
  ebmetadBuilder.def("build", &EBMetaDRestraintBuilder::build);

  // Get more concise name for the template instantiation...
  using PyEBMetaD =
      PyRestraint<plugin::RestraintModule<plugin::EBMetaDRestraint>>;

  // Export a Python class for our parameters struct
  py::class_<plugin::EBMetaDRestraint::input_param_type> ebmetadParams(
      m, "EBMetaDRestraintParams");
  m.def("make_ebmetad_params", &plugin::makeEBMetaDParams);

  // API object to build.
  py::class_<PyEBMetaD, std::shared_ptr<PyEBMetaD>> ebmetad(m,
                                                            "EBMetaDRestraint");
  // EnsembleRestraint can only be created via builder for now.
  ebmetad.def("bind", &PyEBMetaD::bind, "Implement binding protocol");
  /*
   * To implement gmxapi_workspec_1_0, the module needs a function that a
   * Context can import that produces a builder that translates workspec
   * elements for session launching. The object returned by our function needs
  ...
  // in gmxapi workflows. WorkElements will then have namespace: "myplugin" and
  // operation: "linearStop_restraint"
  m.def("ebmetad_restraint",
        [](const py::object element) { return createEBMetaDBuilder(element); });
  //
  // End EBMetaDRestraint
  ///////////////////////////////////////////////////////////////////////////

  //////////////////////////////////////////////////////////////////////////
  // Begin BRERRestraint
  //
  // Define Builder to be returned from brer_restraint Python function defined
  // further down.
  ...
  m.def("make_brer_params", &plugin::makeBRERParams);
