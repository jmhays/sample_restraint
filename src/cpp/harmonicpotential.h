...
public:
  Harmonic(real equilibrium, real springconstant)
      : R0{equilibrium}, k{springconstant} {};

  Harmonic() : Harmonic{0.0, 0.0} {};
  ...
  /*!
   * \brief Calculate harmonic force on particle at position v in reference to
   * position v0.
   *
   ...
   *
   * In the case of a pair of harmonically bonded particles, the force on
   * particle i is evaluated with particle j as the reference point with \code
...
public:
   ...
   */
  HarmonicRestraint(unsigned long int site1, unsigned long int site2, real R0,
                    real k)
      : Harmonic{R0, k}, site1_{site1}, site2_{site2} {};
 ...
 *
 * Objects of this type allow the potential class to be instantiated as the
...
public:
  ...
  HarmonicModule(unsigned long int site1, unsigned long int site2, real R0,
                 real k) {
    site1_ = site1;
    site2_ = site2;
    R0_ = R0;
  ...
  const char *name() override { return "HarmonicModule"; }
  ...
  std::shared_ptr<gmx::IRestraintPotential> getRestraint() override {
    auto restraint =
        std::make_shared<HarmonicRestraint>(site1_, site2_, R0_, k_);
    return restraint;
  }
   ...
   * \param R0
   */
  void setParams(unsigned long int site1, unsigned long int site2, real R0,
                 real k) {
    site1_ = site1;
    site2_ = site2;
    R0_ = R0;
