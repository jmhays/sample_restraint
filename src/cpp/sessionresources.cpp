...
void EnsembleResourceHandle::stop() {
  assert(session_);
  auto signaller =
      gmxapi::getMdrunnerSignal(session_, gmxapi::md::signals::STOP);
...
EnsembleResourceHandle EnsembleResources::getHandle() const {
  ...
  if (!session_) {
    throw gmxapi::ProtocolError("EnsembleResources::getHandle() must not be "
                                "called before setSession() has been called.");
  }
...
void EnsembleResources::setSession(gmxapi::SessionResources *session) {
  if (!session) {
    throw gmxapi::ProtocolError("EnsembleResources::setSession received a null "
                                "SessionResources pointer.");
  }
  session_ = session;
}

void EnsembleResources::setOutputStream(
    std::unique_ptr<gmxapi::session::OutputStream> ostream) {
