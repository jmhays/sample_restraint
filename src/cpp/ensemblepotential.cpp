 ...
 * gmxapi releases, as well as additional code used in implementing the
 * restrained ensemble example workflow.
 *
 * A simpler restraint potential would only update the calculate() function. If
 * a callback function is not needed or desired, remove the callback() code from
 * this file and from ensemblepotential.h
 */
...
/*!
 * \brief Apply a Gaussian blur when building a density grid for a list of
 * values.
 *
...
public:
   ...
   * \param grid Pointer to the container into which to accumulate a blurred
   ...
   *
   *     # Create an empty grid to store magnitudes for points 0.5, 1.0,
   * ..., 10.0. std::vector<double> histogram(20, 0.);
   *
  ...
  void operator()(const std::vector<double> &samples,
    ...
    const double normalization =
        1.0 / (num_samples * sqrt(2.0 * M_PI * sigma_ * sigma_));
    // We aren't doing any filtering of values too far away to contribute
    // meaningfully, which is admittedly wasteful for large sigma...
...
EnsembleHarmonic::EnsembleHarmonic(size_t nbins, double binWidth,
                                   double minDist, double maxDist,
                                   PairHist experimental, unsigned int nSamples,
                                   double samplePeriod, unsigned int nWindows,
                                   double k, double sigma)
    : nBins_{nbins}, binWidth_{binWidth}, minDist_{minDist}, maxDist_{maxDist},
      histogram_(nbins, 0), experimental_{std::move(experimental)},
      nSamples_{nSamples}, currentSample_{0}, samplePeriod_{samplePeriod},
      // In actuality, we have nsamples at (samplePeriod - dt), but we don't
      // have access to dt.
      nextSampleTime_{samplePeriod},
      distanceSamples_(nSamples), nWindows_{nWindows}, currentWindow_{0},
      windowStartTime_{0}, nextWindowUpdateTime_{nSamples * samplePeriod},
      windows_{}, k_{k}, sigma_{sigma} {}

EnsembleHarmonic::EnsembleHarmonic(const input_param_type &params)
    : EnsembleHarmonic(params.nBins, params.binWidth, params.minDist,
                       params.maxDist, params.experimental, params.nSamples,
                       params.samplePeriod, params.nWindows, params.k,
                       params.sigma) {}

//
//
// HERE is the (optional) function that updates the state of the restraint
// periodically. It is called before calculate() once per timestep per
// simulation (on the master rank of a parallelized simulation).
//
//
void EnsembleHarmonic::callback(gmx::Vector v, gmx::Vector v0, double t,
  ...
  //   5. Use handles retained from previous windows to reconstruct the smoothed
  //   working histogram
  if (t >= nextWindowUpdateTime_) {
    // Get next histogram array, recycling old one if available.
    std::unique_ptr<Matrix<double>> new_window =
        gmx::compat::make_unique<Matrix<double>>(1, nBins_);
    std::unique_ptr<Matrix<double>> temp_window;
    if (windows_.size() == nWindows_) {
      // Recycle the oldest window.
      // \todo wrap this in a helper class that manages a buffer we can shuffle
      // through.
      windows_[0].swap(temp_window);
      windows_.erase(windows_.begin());
    } else {
      auto new_temp_window =
          gmx::compat::make_unique<Matrix<double>>(1, nBins_);
      assert(new_temp_window);
      temp_window.swap(new_temp_window);
    }

    // Reduce sampled data for this restraint in this simulation, applying a
    // Gaussian blur to fill a grid.
    ...
    // We can just do the blur locally since there aren't many bins. Bundling
    // these operations for all restraints could give us a chance at some
    // parallelism. We should at least use some threading if we can.

    // We request a handle each time before using resources to make error
    // handling easier if there is a failure in one of the ensemble member
    // processes and to give more freedom to how resources are managed from step
    // to step.
    ...
    // Get new histogram difference. Subtract the experimental distribution to
    // get the values to use in our potential.
    ...
    for (const auto &window : windows_) {
      for (size_t i = 0; i < window->cols(); ++i) {
        histogram_.at(i) +=
            (window->vector()->at(i) - experimental_.at(i)) / windows_.size();
      }
    }

    // Note we do not have the integer timestep available here. Therefore, we
    // can't guarantee that updates occur with the same number of MD steps in
    // each interval, and the interval will effectively lose digits as the
    // simulation progresses, so _update_period should be cleanly representable
    // in binary. When we extract this to a facility, we can look for a part of
    // the code with access to the current timestep.
    windowStartTime_ = t;
    nextWindowUpdateTime_ = nSamples_ * samplePeriod_ + windowStartTime_;
    ++currentWindow_; // This is currently never used. I'm not sure it will be,
                      // either...
...
gmx::PotentialPointData EnsembleHarmonic::calculate(gmx::Vector v,
                                                    gmx::Vector v0, double t) {
  // This is not the vector from v to v0. It is the position of a site
  // at v, relative to the origin v0. This is a potentially confusing
  // convention...
  auto rdiff = v - v0;
  const auto Rsquared = dot(rdiff, rdiff);
  const auto R = sqrt(Rsquared);
...
makeEnsembleParams(size_t nbins, double binWidth, double minDist,
                   double maxDist, const std::vector<double> &experimental,
                   unsigned int nSamples, double samplePeriod,
                   unsigned int nWindows, double k, double sigma) {
...
// Important: Explicitly instantiate a definition for the templated class
// declared in ensemblepotential.h. Failing to do this will cause a linker
// error.
