 ...
 * gmxapi releases, as well as additional code used in implementing the EBMetaD
 * workflow.
 *
 */
...
void writeHistoricalData(std::string filename,
...
EBMetaD::EBMetaD(ForceTable forces, std::vector<unsigned long> distanceCounts,
                 double binWidth, double samplePeriod,
                 std::string historicalDataFilename, double minDist,
                 double maxDist, double k)
    : forces_{std::move(forces)}, distanceCounts_{std::move(distanceCounts)},
      binWidth_{binWidth}, samplePeriod_{samplePeriod},
      historicalDataFilename_{std::move(historicalDataFilename)},
      minDist_{minDist}, maxDist_{maxDist}, k_{k} {};

EBMetaD::EBMetaD(const plugin::EBMetaD::input_param_type &params)
    : EBMetaD(params.forces, params.distanceCounts, params.binWidth,
              params.samplePeriod, params.historicalDataFilename,
              params.minDist, params.maxDist, params.k){};
...
//
//
void EBMetaD::callback(gmx::Vector v, gmx::Vector v0, double t,
...
//
//
gmx::PotentialPointData EBMetaD::calculate(gmx::Vector v, gmx::Vector v0,
                                           double t) {
  ...
  if (R != 0) // Direction of force is ill-defined when v == v0
  {
    ...
    if (R > maxDist_) {
      ...
      /// current distance: these factors depend on 1) the historical data,
      /// i.e., the distances already visited in the simulation
...
makeEBMetaDParams(const ForceTable &forces,
                  std::vector<unsigned long> &distanceCounts, double binWidth,
...
// declared in ebmetad.h. Failing to do this will cause a linker error.
template class ::plugin::RestraintModule<EBMetaDRestraint>;
