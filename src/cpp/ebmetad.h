...
// use the row index for *current* distance, the col index for *historical*
// distances
...
struct ebmetad_input {
  ...
  // This is a table of force magnitudes for faster calculation of the typical
  // MetaDynamics forces. How these force factors should be calculated:
...
makeEBMetaDParams(const ForceTable &forces, std::vector<unsigned long> &counts,
                  double binWidth, double samplePeriod,
                  std::string &historicalDataFilename, double minDist,
                  double maxDist, double k);

/*!
 * \brief a residue-pair bias calculator for use in EBMetaD ensemble
 * simulations.
 *
 ...
 * distribution observed earlier in the simulation trajectory. This is
 * calculated according to doi: 10.1016/j.bpj.2015.05.024.
 *
 */
class EBMetaD {
public:
   ...
   */
  EBMetaD(ForceTable forces, std::vector<unsigned long> counts, double binWidth,
          double samplePeriod, std::string historicalDataFilename,
          double minDist, double maxDist, double k);
...
private:
  ...
  double binWidth_;
  /// A vector of counts of the number of time that particular distance (bin)
  /// has been observed in simulation.
  ...
  ForceTable forces_;
  /// Period (in ps) for updating the distanceCounts_ vector. This determines
  /// how often to "drop a Gaussian" as in the standard MetaDynamics procedure.
  double samplePeriod_;

  /// Define the "boundary conditions" for regions outside of the DEER
  /// distribution. Will be used to apply linear biasing potential to keep
  /// simulation probability mass inside the DEER region.
  double minDist_;
  double maxDist_;
  double k_;
};
...
public:
  ...
  EBMetaDRestraint(std::vector<unsigned long> sites,
      ...
      : EBMetaD(params), sites_{std::move(sites)}, resources_{
                                                       std::move(resources)} {}
...
extern template class RestraintModule<EBMetaDRestraint>;
