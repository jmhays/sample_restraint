 ...
 *
 * The restraint implemented here uses a facility provided by gmxapi to perform
 * averaging of some array data across an ensemble of simulations. Simpler pair
 * restraints can use less of this example code.
 *
 * Contains a lot of boiler plate that is being generalized and migrate out of
 * this file, but other pair restraints can be implemented by following the
 * example in this and ``ensemblepotential.cpp``. The ``CMakeLists.txt`` file
 * will need to be updated if you add additional source files, and
 * ``src/pythonmodule/export_plugin.cpp`` will need to be updated if you add or
 * change the name of potentials.
 */
...
// We do not require C++14, so we have a back-ported C++14 feature for C++11
// code.
...
/*!
 * \brief a residue-pair bias calculator for use in restrained-ensemble
 * simulations.
 *
 * Applies a force between two sites according to the difference between an
 * experimentally observed site pair distance distribution and the distance
 * distribution observed earlier in the simulation trajectory. The sampled
 * distribution is averaged from the previous `nwindows` histograms from all
 * ensemble members. Each window contains a histogram populated with `nsamples`
 * distances recorded at `sample_period` step intervals.
 *
 * \internal
 * During a the window_update_period steps of a window, the potential applied is
 * a harmonic function of the difference between the sampled and experimental
 * histograms. At the beginning of the window, this difference is found and a
...
public:
   ...
   *
   * This constructor is called once per simulation per GROMACS process. Note
   * that until gmxapi 0.0.8 there is only one instance per simulation in a
   * thread-MPI simulation.
   *
   * \param params
   */
   ...
   * \param sigma
   */
  EnsembleHarmonic(size_t nbins, double binWidth, double minDist,
                   double maxDist, PairHist experimental, unsigned int nSamples,
                   double samplePeriod, unsigned int nWindows, double k,
                   double sigma);

  /*!
   * \brief Evaluates the pair restraint potential.
   *
   * In parallel simulations, the gmxapi framework does not make guarantees
   * about where or how many times this function is called. It should be simple
   * and stateless; it should not update class member data (see
   * ``ensemblepotential.cpp``. For a more controlled API hook and to manage
   * state in the object, use ``callback()``.
   *
   ...
   */
  // Implementation note for the future: If dispatching this virtual function is
  // not fast enough, the compiler may be able to better optimize a free
  // function that receives the current restraint as an argument.
  gmx::PotentialPointData calculate(gmx::Vector v, gmx::Vector v0,
                                    gmx_unused double t);

  /*!
   * \brief An update function to be called on the simulation master rank/thread
   * periodically by the Restraint framework.
   *
   * Defining this function in a plugin potential is optional. If the function
   * is defined, the restraint framework calls this function (on the first rank
   * only in a parallel simulation) before calling calculate().
   *
   * The callback may use resources provided by the Session in the callback to
   * perform updates to the local or global state of an ensemble of simulations.
   * Future gmxapi releases will include additional optimizations, allowing
   * call-back frequency to be expressed, and more general Session resources, as
   * well as more flexible call signatures.
   */
...
private:
  ...
  double minDist_;
  double maxDist_;
  /// Smoothed historic distribution for this restraint. An element of the array
  /// of restraints in this simulation.
 ...
 */
class EnsembleRestraint : public ::gmx::IRestraintPotential,
                          private EnsembleHarmonic {
public:
  ...
  EnsembleRestraint(std::vector<unsigned long> sites,
      ...
      : EnsembleHarmonic(params), sites_{std::move(sites)},
   ...
   *
   * This boilerplate will disappear into the Restraint template in an upcoming
   * gmxapi release.
   */
